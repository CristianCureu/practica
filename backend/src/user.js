...
exports.decode = exports.passwordReset = exports.passwordRecover = exports.validateToken = exports.registerUser = exports.logoutUser = exports.loginUser = void 0;
const jwt = require("jsonwebtoken");
const sql_1 = require("./sql");
const query_1 = require("./query");
const email_1 = require("./email");
const ms = require("ms");
const TOKEN_SECRET_KEY = process.env.TOKEN_SECRET_KEY || "5981c9ee-ade4-412f-ae7d-70071fdd708f55a45f";
...
async function loginUser(ctx) {
    try {
        let { recordset: [user], } = await (0, sql_1.sql)(query_1.loginUserQuery, ctx.request.body);
...
async function registerUser(ctx) {
    ...
    let { name, email, password, passwordRepeat } = ctx.request.body;
    let validationErrors = ['<div class="row"></div>'];
    if (!name)
        validationErrors.push("<b>Your name</b> is required");
    if (!email)
        validationErrors.push("<b>Email</b> is required");
    if (!password)
        validationErrors.push("You must have a <b>password</b>");
    if (password != passwordRepeat)
        validationErrors.push("<b>Password</b> and <b>Password repeat</b> don't match");
    if (validationErrors.length > 1) {
        return ctx.throw(400, validationErrors.join("<br/>"));
    }
    try {
        let { recordset: [user], } = await (0, sql_1.sql)(query_1.registerUserQuery, ctx.request.body);
...
`;
        ...
        await (0, email_1.default)(email, "Activare cont laguna delivery", html, text);
...
async function validateToken(ctx) {
    try {
        await (0, sql_1.sql)(query_1.validateTokenQuery, ctx.request.body);
        ctx.body = { token: "OK" };
    }
    catch (e) {
        ctx.throw(400, e);
...
async function passwordRecover(ctx) {
    try {
        let { email } = ctx.request.body;
        let { recordset: [user], } = await (0, sql_1.sql)(query_1.passwordRecoverQuery, ctx.request.body);
        let name = user.name;
...
<body>
  ...
  <p>Echipa laguna delivery</p>
        ...
        await (0, email_1.default)(email, "Resetare parolă laguna delivery", html, text);
        ctx.body = { sendMail: "OK" };
    }
    catch (e) {
        console.error(e);
...
async function passwordReset(ctx) {
    ...
    let validationErrors = ['<div class="row"></div>'];
    if (!email)
        validationErrors.push("Reset <b>token</b> is required");
    if (!password)
        validationErrors.push("You must have a <b>password</b>");
    if (password != passwordRepeat)
        validationErrors.push("<b>Password</b> and <b>Password repeat</b> don't match");
