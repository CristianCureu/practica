...
async function getDbModels() {
    let { recordset: tables } = await (0, sql_1.sql)(getTables);
    let { recordset: columns } = await (0, sql_1.sql)(getColumns);
    let dbo = {};
    let entity = {};
    ...
    await fs_1.promises.writeFile('./src/app/shared/models.ts', models);
    await fs_1.promises.writeFile('./api/src/models.ts', models);
    return { dbo };
}
function jsType(sqlType) {
    if (~['nvarchar', 'varchar', 'nchar', 'char', 'ntext', 'text', 'uniqueidentifier'].indexOf(sqlType))
        return 'string';
    if (~['numeric', 'decimal', 'bigint', 'int', 'smallint', 'tinyint', 'float', 'real', 'money', 'smallmoney'].indexOf(sqlType))
        return 'number';
    if (~['datetimeoffset', 'datetime2', 'datetime', 'smalldatetime', 'date', 'time'].indexOf(sqlType))
        return 'Date';
    if (~['bit'].indexOf(sqlType))
        return 'boolean | number';
    throw new Error(`Unknown "${sqlType}" sqlType`);
}
function optional(key) {
    if (~['owner', 'stamp'].indexOf(key))
        return `${key}?`;
    return key;
}
getDbModels()
